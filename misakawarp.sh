...
install_wgcf(){
    ...
    if [[ $SYSTEM == "CentOS" ]]; then
        ...
        if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]]; then
            if [[ $vpsvirt =~ "kvm"|"xen"|"microsoft"|"vmware"|"qemu" ]]; then
                ...
                wget -N --no-check-certificate https://raw.githubusercontents.com/tpe28/Misaka-WARP-Script/master/files/wireguard-go-$(archAffix) -O /usr/bin/wireguard-go
        ...
        if [[ $vsid == 9 ]] && [[ -z $(type -P resolvconf) ]]; then
            ...
            wget -N https://raw.githubusercontents.com/tpe28/Misaka-WARP-Script/master/files/resolvconf -O /usr/sbin/resolvconf
    ...
    wget -N --no-check-certificate https://raw.githubusercontents.com/tpe28/Misaka-WARP-Script/master/files/wgcf-latest-linux-$(archAffix) -O /usr/local/bin/wgcf
